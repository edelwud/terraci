...
import (
	...
	"github.com/edelwud/terraci/plugins/cost/engine/aws"
...
func (e *Estimator) EstimateModule(ctx context.Context, modulePath, region string) (*costtypes.ModuleCost, error) {
		...
		if rc.Action == plan.ActionUpdate || rc.Action == plan.ActionReplace {
			...
			if handler, ok := e.registry.GetHandler(rc.Type); ok && beforeAttrs != nil {
				...
				case aws.CostCategoryStandard:
					beforeResult := e.estimateStandardResource(ctx, handler, beforeAttrs, region, costtypes.ResourceCost{})
...
func (e *Estimator) aggregateCost(result *costtypes.ModuleCost, rc costtypes.ResourceCost, action string) {
	if rc.IsUnsupported() {
		result.Unsupported++
		return
	}
...
func groupByModule(resources []costtypes.ResourceCost) []costtypes.SubmoduleCost {
	// Step 1: group resources by exact ModuleAddr
	type flatGroup struct {
		addr      string
		resources []costtypes.ResourceCost
		cost      float64
	}
	...
	nodes := make(map[string]*costtypes.SubmoduleCost, len(order))
	for _, addr := range order {
		g := groups[addr]
		nodes[addr] = &costtypes.SubmoduleCost{
			ModuleAddr:  addr,
			MonthlyCost: g.cost,
	...
	var roots []costtypes.SubmoduleCost
...
func findParentAddr(addr string, nodes map[string]*costtypes.SubmoduleCost) string {
...
func (e *Estimator) EstimateModules(ctx context.Context, modulePaths []string, regions map[string]string) (*costtypes.EstimateResult, error) {
	results := make([]costtypes.ModuleCost, len(modulePaths))
	...
	for i, modulePath := range modulePaths {
		...
		g.Go(func() error {
			...
			if err != nil {
				...
				results[i] = costtypes.ModuleCost{
					ModuleID:   modulePath,
					ModulePath: modulePath,
					Error:      err.Error(),
				}
	...
	result := &costtypes.EstimateResult{
		Modules:     results,
		Currency:    "USD",
...
func (e *Estimator) estimateResourceFromAttrs(ctx context.Context, resourceType, address, name, moduleAddr, region string, attrs map[string]any) costtypes.ResourceCost {
	result := costtypes.ResourceCost{
		Address:    address,
	...
	if !ok {
		result.ErrorKind = costtypes.CostErrorNoHandler
	...
	switch handler.Category() {
	case aws.CostCategoryUsageBased:
		result.ErrorKind = costtypes.CostErrorUsageBased
...
func (e *Estimator) estimateStandardResource(ctx context.Context, handler aws.ResourceHandler, attrs map[string]any, region string, result costtypes.ResourceCost) costtypes.ResourceCost {
	lookup, err := handler.BuildLookup(region, attrs)
	if err != nil {
		result.ErrorKind = costtypes.CostErrorLookupFailed
	...
	if err != nil {
		...
		result.ErrorKind = costtypes.CostErrorAPIFailure
	...
	if err != nil {
		...
		result.ErrorKind = costtypes.CostErrorNoPrice
