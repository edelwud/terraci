...
// goldenCase describes a single deterministic generator scenario whose YAML
// output is fixed in testdata/golden/<name>.yaml. Add a new case here when a
// feature should be locked against silent regression at the rendered-YAML
// boundary. Run `go test -run TestGoldenYAML -update ./plugins/gitlab/...`
...
func TestGoldenYAML(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pipelineDoc := scenario.generate()
			yamlBytes, err := pipelineDoc.ToYAML()
			if err != nil {
