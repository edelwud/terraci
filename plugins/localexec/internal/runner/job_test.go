...
func TestOperationDispatcherRoutesTerraformOperations(t *testing.T) {
	...
	if err := dispatcher.Run(context.Background(), plan); err != nil {
		t.Fatalf("Run(plan) error = %v", err)
	}
	if err := dispatcher.Run(context.Background(), apply); err != nil {
		t.Fatalf("Run(apply) error = %v", err)
	}

	if !reflect.DeepEqual(terraform.plans, []string{plan.Name()}) {
		t.Fatalf("plans = %v, want [%s]", terraform.plans, plan.Name())
	}
	if !reflect.DeepEqual(terraform.applies, []string{apply.Name()}) {
		t.Fatalf("applies = %v, want [%s]", terraform.applies, apply.Name())
	}
...
func TestOperationDispatcherRunsCommandOperationsWithJobMetadata(t *testing.T) {
	...
	if err := dispatcher.Run(context.Background(), commandJob); err != nil {
		t.Fatalf("Run() error = %v", err)
	}

	if got, want := commands.commands, commandJob.Operation().Commands(); !reflect.DeepEqual(got, want) {
		t.Fatalf("commands = %v, want %v", got, want)
	}
	for _, spec := range commands.specs {
		if spec.JobName != commandJob.Name() {
			t.Fatalf("job name = %q, want %q", spec.JobName, commandJob.Name())
		}
...
func TestJobRunnerStandaloneCommandsPropagateAllowFailure(t *testing.T) {
	...
	if err := runner.Run(context.Background(), commandJob); err != nil {
		t.Fatalf("Run() error = %v", err)
	}
	...
	if commands.specs[0].JobName != commandJob.Name() {
		t.Fatalf("job name = %q, want %q", commands.specs[0].JobName, commandJob.Name())
	}
