...
```
dry run results
  modules discovered                          total: 15
  modules to process                          affected: 8
  pipeline stages                             stages: 6
  pipeline jobs                               jobs: 16

execution order
  level 0                                     modules: [vpc, iam]
  level 1                                     modules: [eks, rds, cache]
  level 2                                     modules: [app-backend, app-frontend]
  level 3                                     modules: [monitoring]
```
...
When writing to a file, the output begins with a header:

```yaml
# Generated by terraci vX.Y.Z
# DO NOT EDIT
```
