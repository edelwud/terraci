...
Секция `github` управляет генерацией GitHub Actions workflow. Эта секция используется, когда провайдер — `github` (задаётся через `provider: github` в конфиге или автоопределяется из переменной окружения `GITHUB_ACTIONS`). Когда провайдер — `gitlab`, эта секция не используется и вместо неё применяется секция `gitlab`. См. [Конфигурация GitLab CI](/ru/config/gitlab) для эквивалента GitLab.
