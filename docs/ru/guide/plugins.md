...
| `update` | Проверка версий провайдеров и модулей Terraform | Да (`enabled: true`) |
...
Плагины **cost**, **policy** и **update** требуют явной активации:
...
|------------|---------|---------|
| `CommandProvider` | CLI-субкоманды (`terraci cost` и т.д.) | cost, policy, summary, update |
| `PipelineContributor` | Добавление шагов/джобов в pipeline IR | cost, policy, summary |
| `InitContributor` | Поля формы для `terraci init` | gitlab, github, cost, policy, summary, update |
...
| `RuntimeProvider` | Ленивая инициализация тяжёлых зависимостей | cost, policy, update |
| `Preflightable` | Дешёвая валидация при старте | gitlab, github, git, cost, policy, update |
...
| Обновление зависимостей | [config/update](/ru/config/update) |
