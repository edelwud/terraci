...
type PolicyConfig struct {
	...
	Sources []PolicySource `yaml:"sources,omitempty" json:"sources,omitempty" jsonschema:"description=Policy sources (local path, git, or OCI)"`
	...
	OnFailure PolicyAction `yaml:"on_failure,omitempty" json:"on_failure,omitempty" jsonschema:"description=Behavior on policy failure,enum=block,enum=warn,default=block"`

	// OnWarning defines behavior for warnings: warn or ignore
	OnWarning PolicyAction `yaml:"on_warning,omitempty" json:"on_warning,omitempty" jsonschema:"description=Behavior on policy warnings,enum=warn,enum=ignore,default=warn"`
	...
	Overwrites []PolicyOverwrite `yaml:"overwrites,omitempty" json:"overwrites,omitempty" jsonschema:"description=Per-module policy configuration overrides"`
...
// PolicyAction defines the action to take on policy violations
type PolicyAction string

const (
	// PolicyActionBlock fails the pipeline on violation
	PolicyActionBlock PolicyAction = "block"
	// PolicyActionWarn only warns but doesn't fail
	PolicyActionWarn PolicyAction = "warn"
	// PolicyActionIgnore ignores the violation
	PolicyActionIgnore PolicyAction = "ignore"
)

// PolicySource defines a source for policy files
...
// PolicyOverwrite defines per-module policy overrides
type PolicyOverwrite struct {
	...
	OnFailure PolicyAction `yaml:"on_failure,omitempty" json:"on_failure,omitempty" jsonschema:"description=Override failure behavior for matching modules"`

	// OnWarning overrides warning behavior for matching modules
	OnWarning PolicyAction `yaml:"on_warning,omitempty" json:"on_warning,omitempty" jsonschema:"description=Override warning behavior for matching modules"`
}

// GetEffectiveConfig returns the effective policy config for a module path
// by applying overwrites that match the path
func (p *PolicyConfig) GetEffectiveConfig(modulePath string) *PolicyConfig {
	if p == nil {
		return nil
	}
...
func (p *PolicyConfig) Validate() error {
	...
	for i, ow := range p.Overwrites {
		...
		if ow.OnFailure != "" && ow.OnFailure != PolicyActionBlock && ow.OnFailure != PolicyActionWarn {
			return fmt.Errorf("overwrites[%d].on_failure must be 'block' or 'warn'", i)
		}
		if ow.OnWarning != "" && ow.OnWarning != PolicyActionWarn && ow.OnWarning != PolicyActionIgnore {
