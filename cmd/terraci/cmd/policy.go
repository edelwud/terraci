...
Example:
  ...
  terraci policy pull --output ./my-policies`,
		RunE: func(_ *cobra.Command, _ []string) error {
			if app.Config.Policy == nil || !app.Config.Policy.Enabled {
			...
			if policyOutput != "" {
				app.Config.Policy.CacheDir = policyOutput
			}

			puller, err := policy.NewPuller(app.Config.Policy, app.WorkDir)
			if err != nil {
...
Example:
  ...
  terraci policy check --output json`,
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			// Create checker
			checker := policy.NewChecker(app.Config.Policy, policyDirs, app.WorkDir)
