...
Compile-time plugins via `init()` + blank import (Caddy/database-sql pattern). Plugins register factories via `registry.RegisterFactory()`, and `cmd/terraci/internal/runflow` creates a fresh `*registry.Registry` for each command run. `*Registry` implements the narrow plugin resolver interfaces and owns lifecycle discovery through facades/snapshots (`Commands`, `DecodeConfig`, `ExtensionDefinitions`, `RunPreflight`, `VersionSnapshot`, `InitWizardSnapshot`). Core types (interfaces, BasePlugin, AppContext) live in `pkg/plugin`; plugin catalog and per-command registries live in `pkg/plugin/registry`; init wizard types in `pkg/plugin/initwiz`.
...
Command handlers should stay as a thin boundary: resolve `cmdCtx` and the command-scoped plugin with `plugin.CommandPlugin[T]`, read runtime state through `cmdCtx.AppContext()`, gate configured plugins with `plugin.RequireEnabled`, parse cobra flags into a typed request, then call a usecase method. `runflow.Prepared` owns the framework command binding and planning snapshot; `AppContext` is runtime context only and does not carry command lookup or pipeline contributions. The canonical shape is:
...
- `pkg/ci/citest`: `AssertRenderedReportContract`, `AssertPublishArtifactsContract`, `PublishReport`, `NewStaticReportLoader`, and rendered-section/report builders.
...
`pkg/ci/` contains shared CI-domain types including provider-shared config such as `Image` (with YAML shorthand). `ci.Report` is the typed report contract shared by cost/policy/tfupdate/summary; `ci.ReportStore` is the narrow report artifact port (`LoadReports` + `PublishArtifacts`) for in-memory and file-backed persistence. Plan-aware producers carry one immutable `PlanResultCollection` into `plugin.NewArtifactRun`; reports carry provenance derived from `ArtifactRun.Artifact()` so local consumers can validate current artifacts. Consumers read reports through immutable `ci.ReportCollection` values. Both gitlab and github internal packages use type aliases to these.
...
`ci.PlanResult` is the canonical representation of one module's plan outcome — used both in-memory and on disk; `ci.PlanResultCollection` aggregates them with a stable fingerprint. Build plan results and collections through constructors and read them through getters; do not construct or mutate plan result structs by hand.
...
3. Build `summaryengine.SummarySnapshot` from `PlanResultCollection + ReportCollection`
4. `summaryengine.ResolveLabels()` expands static/module/resource labels from the plan-result collection
5. `summaryengine.ComposeCommentWithOptions()` renders markdown from the snapshot and embeds managed-label metadata
6. `appCtx.CIResolver().ResolveCIProvider()` → `NewCommentService()` → `UpsertComment(ctx, body)`
7. If the comment service implements `ci.ManagedLabelService`, sync labels by removing only prior TerraCI-managed labels absent from the current run and adding current labels
...
- **Init extension contracts**: init wizard plugins define package-local `initwiz.StateKey[T]` and `config.ExtensionKey` values, build fields through `initwiz.NewStringField` / `NewBoolField` / `NewSelectField`, and return typed config structs/maps through `initwiz.NewInitContribution`. Core owns YAML node encoding and defensive copies; initflow owns duplicate detection and final assembly through `config.ExtensionValueSet`. Do not return loose extension maps from plugin init code.
- **Report artifact lifecycle**: plan-aware producers use `PlanResultCollection -> plugin.NewArtifactRun -> ci.NewRenderedReport -> ci.NewArtifactPublication -> appCtx.Reports().PublishArtifacts(...)`. `ReportStore.PublishArtifacts` persists `ci.RawResults(...)` and removes stale reports on nil/build errors. Report-only producers use `ci.NoResults()` in the publication intent. Consumers call `LoadReports(ctx)` and receive an immutable `ci.ReportCollection`. Summary composition uses `PlanResultCollection + ReportCollection -> SummarySnapshot -> renderer/comment`; raw report/result slices stay inside small local iteration helpers.
