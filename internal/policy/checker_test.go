...
`), 0o644); err != nil {
	...
	cfg := &config.PolicyConfig{
		...
		Overwrites: []config.PolicyOverwrite{
			{Match: "**/sandbox/**", OnFailure: config.PolicyActionWarn},
		},
	}
...
`), 0o644); err != nil {
	...
	cfg := &config.PolicyConfig{
		Enabled:   true,
		OnFailure: config.PolicyActionWarn, // global: warn
		Overwrites: []config.PolicyOverwrite{
			{Match: "**/prod/**", OnFailure: config.PolicyActionBlock}, // prod: block
		},
	}
	checker := NewChecker(cfg, []string{policyDir}, tmpDir)
