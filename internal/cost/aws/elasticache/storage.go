...
func dataTieringCost(price *pricing.Price, index *pricing.PriceIndex, region string, nodeCount int) float64 {
	...
	if index != nil && region != "" {
		if looked, found := lookupElastiCachePrice(index, region, "Cache Storage", aws.ResolveUsagePrefix(region)+"-DataTiering:StorageUsage"); found {
			costPerGB = looked
		}
	}
...
func backupStorageCost(price *pricing.Price, index *pricing.PriceIndex, region string, nodeCount, snapshotRetention int) float64 {
	...
	if index != nil && region != "" {
		if looked, found := lookupElastiCachePrice(index, region, "Storage Snapshot", aws.ResolveUsagePrefix(region)+"-BackupUsage"); found {
			costPerGB = looked
		}
	}
